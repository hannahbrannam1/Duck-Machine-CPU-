...
class BitField(object):
    ...
    def __init__(self, from_bit: int, to_bit: int) -> None:
        ...
        self.mask = 0
        for i in range(self.from_bit - 1, self.to_bit):
    ...
    def insert(self, value: int, word: int) -> int:
        ...
        shifted_value = value << self.from_bit
        mask = self.mask << self.from_bit
        return (word & ~mask) | (shifted_value & mask)

    def extract_signed(self, word: int) -> int:
        """Extract bits in bitfield as a signed integer."""
        unsigned = self.extract(word)
        return sign_extend(unsigned, self.to_bit - self.from_bit + 1)
